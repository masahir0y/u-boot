...
static void ave_halt(struct ave_private *priv)
{
	u32 val;
	int ret;
...
static int ave_init(struct ave_private *priv)
{
	uintptr_t paddr;
	u32 val;
	int i;
...
static int ave_write_hwaddr(struct ave_private *priv, u8 *mac)
{
...
static int ave_send(struct ave_private *priv, void *packet, int length)
{
	u32 val;
	void *ptr = packet;
	int count;
...
static int ave_recv(struct ave_private *priv, uchar **packetp)
{
	uchar *ptr;
	int length = 0;
	u32 cmdsts;
...
static int ave_free_packet(struct ave_private *priv)
{
...
static int ave_dm_start(struct udevice *dev)
{
...
static void ave_dm_stop(struct udevice *dev)
{
	struct ave_private *priv = dev_get_priv(dev);
...
static int ave_dm_send(struct udevice *dev, void *packet, int length)
{
	return ave_send(dev_get_priv(dev), packet, length);
}

static int ave_dm_recv(struct udevice *dev, int flags, uchar **packetp)
{
	return ave_recv(dev_get_priv(dev), packetp);
}

static int ave_dm_free_packet(struct udevice *dev, uchar *packet, int length)
{
	return ave_free_packet(dev_get_priv(dev));
}

static int ave_dm_write_hwaddr(struct udevice *dev)
{
	struct eth_pdata *pdata = dev_get_platdata(dev);
	struct ave_private *priv = dev_get_priv(dev);

	return ave_write_hwaddr(priv, pdata->enetaddr);
}
...
static int ave_ofdata_to_platdata(struct udevice *dev)
{
	...
	priv->nr_clk = 0;
	ret = ofnode_count_phandle_with_args(dev_ofnode(dev), "clocks",
					     "#clock-cells");
	if (ret > 0) {
		priv->nr_clk = ret;
		priv->clk = devm_kcalloc(dev, priv->nr_clk, sizeof(struct clk),
					 GFP_KERNEL);
		if (!priv->clk)
			return -ENOMEM;

		for (nclk = 0; nclk < priv->nr_clk; nclk++) {
			ret = clk_get_by_index(dev, nclk, &priv->clk[nclk]);
			if (ret) {
		...
		dev_err(dev, "Illegal clocks property: %d\n", ret);
		return ret;
	}

	ret = ofnode_count_phandle_with_args(dev_ofnode(dev), "resets",
					     "#reset-cells");
	if (ret > 0) {
		priv->nr_rst = ret;
		priv->rst = devm_kcalloc(dev, priv->nr_rst,
		...
		for (nrst = 0; nrst < priv->nr_rst; nrst++) {
			ret = reset_get_by_index(dev, nrst, &priv->rst[nrst]);
			if (ret) {
		...
		dev_err(dev, "Illegal resets property: %d\n", ret);
		goto out_clk_free;
	}
...
out_reset_free:
	while (nrst--)
		reset_free(&priv->rst[nrst]);
out_clk_free:
	while (nclk--)
		clk_free(&priv->clk[nclk]);
...
static int ave_probe(struct udevice *dev)
{
	...
	for (nrst = 0; nrst < priv->nr_rst; nrst++) {
		ret = reset_deassert(&priv->rst[nrst]);
		if (ret) {
...
out_mdiobus_release:
	...
	mdio_free(priv->bus);
out_reset_release:
	reset_release_all(priv->rst, nrst);
out_clk_release:
	clk_release_all(priv->clk, nclk);
...
static int ave_remove(struct udevice *dev)
{
	...
	mdio_unregister(priv->bus);
	mdio_free(priv->bus);
	reset_release_all(priv->rst, priv->nr_rst);
	clk_release_all(priv->clk, priv->nr_clk);
...
static const struct eth_ops ave_ops = {
	...
	.send         = ave_dm_send,
	.recv         = ave_dm_recv,
	.free_pkt     = ave_dm_free_packet,
	.write_hwaddr = ave_dm_write_hwaddr,
};
